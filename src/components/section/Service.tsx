    ...
    >
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-screen-xl">
        {ServiceList.map(({ title, des, img }, idx) => (
          <InteractiveCard
            key={idx}
            className="flex flex-col items-center md:items-start justify-between border border-gray-800 rounded-2xl p-6 transition-all duration-500  hover:border-blue-500 animated-gradient-border"
          >
            {/* Image */}
            ...
            {/* Description */}
            <p className="text-gray-400 text-sm leading-relaxed mb-4 hover:text-white relative z-10">
              {des}
            </p>
